...
const TasksList = () => {
  ...
  return (
    <div className='flex flex-col gap-4 relative'>
            ...
            tasks.map((task)=>{
                return task?.isCompleted === false && <div className='h-16 border-t-2 border-b-2 flex items-center justify-between' key={task?.id}>
                        <div className='flex gap-3 items-center text-center pl-4'>
                            <input type="checkbox" className={`w-4 h-4 cursor-pointer ${task?.isCompleted && "peer-checked:bg-green-600"}`} checked={task?.isCompleted} onClick={()=> markCompleteTaskHandler(task)}/>
                            <h3>{task?.title}</h3>
                        </div>
                    ...
                    tasks.map((task)=>{
                        return task?.isCompleted === true &&  <div className='h-16 border-t-2 border-b-2 flex items-center justify-between' key={task?.id}>
                                <div className='flex gap-6 items-center justify-center pl-4'>
                                    <input type="checkbox" className='w-4 h-4 cursor-pointer' checked={task?.isCompleted} onClick={()=> markCompleteTaskHandler(task)}/>
        ...
        {/* update Task popup */}
        <div className={`flex flex-col mx-auto gap-4 absolute md:w-[60%] z-10 bg-[#d8ebda] h-52 ${isUpdated ? "flex md:ml-[20%] top-4": "hidden"} bottom-0 w-full rounded-md transition-all w-full`}>
